...
use leptos::{create_signal, ReadSignal, SignalGet, SignalSet};
...
pub fn net_monitor() -> (ReadSignal<Option<ActiveConnection>>, ReadSignal<Vec<Vpn>>) {
    let (active_connection, set_active_connection) = create_signal(None);
    let (vpn_list, set_vpn_list) = create_signal(vec!());

    spawn(async move {
        let wifi = active_connection.get().and_then(|c| match c {
        ...
        if let Some((ssid, device)) = wifi {
            ...
            set_active_connection.set(Some(ActiveConnection::Wifi {
                ssid,
                device,
                signal: new_signal,
            }));
    ...
    spawn(async move {
        set_active_connection.set(get_active_connection().await);
        set_vpn_list.set(get_vpn().await);

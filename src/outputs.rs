...
impl Outputs {
    ...
    pub fn get_height(inset: f32, scale_factor: f64) -> f64 {
        (HEIGHT - inset as f64) * scale_factor
    }
    ...
    pub fn exclusive_zone(layout: BarLayout, position: Position, scale_factor: f64) -> i32 {
        let height = Self::get_height(layout.appearance.inset, scale_factor);
            ...
            if *oid == Some(target) {
                info.as_ref().and_then(|i| {
                    i.output_logical_height.map(|h| {
                        let bar =
                            Self::get_height(i.layout.appearance.inset, i.scale_factor) as u32;

    ...
    Renderer: iced::advanced::Renderer,
{
    ...
    fn layout(
        ...
        if !self.animated {
            // Animations disabled: snap to the target value.
            let target = if self.open { 1.0 } else { 0.0 };
            state.progress_anim = Animation::new(target);
            state.last_open = self.open;
            state.initialized = true;
        } else if !state.initialized {
            // First layout: start at 0 (closed) and animate to 1 if open,
            // matching the freshly-allocated surface entering the scene.
    ...
    fn draw(
        ...
        // Backdrop alpha tracks progress so it fades in/out with the menu.

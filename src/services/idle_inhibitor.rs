...
// ── guido adapter ────────────────────────────────────────────────────────────
// Everything above is upstream ashell's idle_inhibitor, copied verbatim.
// The manager is synchronous (upstream drives it from the iced update
// loop); here a guido service owns it and mirrors state into a signal.
...
#[derive(Clone)]
pub enum IdleInhibitorCmd {
    Toggle,
}

pub fn create() -> (IdleInhibitorDataSignals, Service<IdleInhibitorCmd>) {
    let data = IdleInhibitorDataSignals::new(IdleInhibitorData::default());
    let writers = data.writers();

    let svc = create_service::<IdleInhibitorCmd, _, _>(move |mut rx, _ctx| async move {
        let mut manager = IdleInhibitorManager::new();
        while let Some(cmd) = rx.recv().await {
            match cmd {
                IdleInhibitorCmd::Toggle => {
                    if let Some(m) = &mut manager {
                        m.toggle();
                        writers.inhibited.set(m.is_inhibited());
                    }
                }
            }
        }

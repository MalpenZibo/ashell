...
The `copr-build.yml` workflow submits builds to Fedora's COPR (Community Projects) build system, making ashell available via `dnf`:

```bash
sudo dnf copr enable <owner>/ashell

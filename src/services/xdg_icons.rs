//! XDG icon resolution for tray items.
//!
//! Tray applications report icon *names* that must be resolved against the
//! installed icon themes. Names are frequently broken (wrong case, vendor
//! suffixes like `-linux-64`, WM-class instead of icon name), so lookup
//! falls back through: direct theme lookup → similar-name candidates →
//! desktop-file index → prefix match. Ported from ashell.
...
pub enum XdgIcon {
    /// Raster image (file path or raw RGBA pixmap)
    Image(ImageSource),
    /// SVG image
    Svg(ImageSource),
}
...
fn icon_from_path(path: PathBuf) -> Option<XdgIcon> {
    if path.extension().is_some_and(|ext| ext == "svg") {
        ...
        Some(XdgIcon::Svg(ImageSource::SvgPath(path)))
    } else {
        debug!("raster icon found. Path: {path:?}");

        Some(XdgIcon::Image(ImageSource::Path(path)))
    }
}

...
use crate::{
    ...
    menu::{self}, // modules::settings::power::power_menu,
                  // password_dialog,
                  // services::{
...
use iced::{
    ...
    window::Id,
    Alignment, Background, Border, Element, Length, Subscription, Task, Theme,
};
...
impl Settings {
    pub fn update(
        ...
        match message {
            ...
            Message::Lock => {
              ...
              //                 Task::none()
              //             };
              //             Task::batch(vec![menu.unset_keyboard_interactivity(), network_command])
              //         } else {
              //             Task::none()
              //         }
              //     }
    ...
    pub fn menu_view(&self, config: &SettingsModuleConfig) -> Element<Message> {
        column![text("menu settings")].into()
